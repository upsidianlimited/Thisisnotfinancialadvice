...
function AuthScreen({ onLogin }) {
  ...
  const handleSignup = async () => {
    ...
    if (authErr && !authErr.message.includes("already registered")) { flash(authErr.message); setLoading(false); return; }
  ...
  const handleForgot = async () => {
    ...
    if (error) flash("Could not send reset email. Try again.");
...
function AccessScreen() {
  ...
  const handleAdd = async () => {
    ...
    if (ok) { setInput(""); load(); setMsg({ text: `${email} added.`, type: "ok" }); }
    else setMsg({ text: "Already on the list or error occurred.", type: "err" });
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overscrollBehavior: "contain", WebkitOverflowScrolling: "touch" }}>
      <div style={{ padding: "24px 20px 40px" }}>
        <div style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: T.textMute, marginBottom: 6 }}>Access List</div>
        <div style={{ fontSize: 13, color: T.textMute, marginBottom: 24 }}>Only these emails can create an account.</div>
        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: "16px", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            />
            <button onClick={handleAdd} style={{ padding: "11px 18px", background: T.accent, color: "#080A0F", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: "0.05em", whiteSpace: "nowrap" }}>
              Add
            </button>
          </div>
          {msg.text && (
            <div style={{ fontSize: 12, marginTop: 10, color: msg.type === "ok" ? T.success : T.danger }}>
              {msg.text}
            </div>
          )}
        </div>
      ...
      <div style={{ padding: "24px 20px 40px" }}>
        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: "16px 20px", marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => { setPwOpen(v => !v); setPwErr(""); setPwOk(""); }} style={{ background: "none", border: "none", color: T.accent, fontSize: 13, cursor: "pointer" }}>
...
function MainApp({ user, onLogout }) {
  ...
  return (
    <div style={shell}>
      ...
      {tab === "access"  && isAdmin && <AccessScreen />}
